...
class PromptsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create prompt" do
    assert_difference("Prompt.count") do
      post prompts_url, params: { prompt: { description: @prompt.description, title: @prompt.title } }
    end
  ...
  test "should update prompt" do
    patch prompt_url(@prompt), params: { prompt: { description: @prompt.description, title: @prompt.title } }
