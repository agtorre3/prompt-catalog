class PromptsController < ApplicationController
  ...
  def index
    @prompts = Prompt.includes(:tags, :characters, :relationships, :character_traits, :user).all
  end
  ...
  def set_prompt
    @prompt = Prompt.includes(:characters, :user, :tags, :relationships, character_traits: [:character, :trait]).find(params[:id])
  end

  # Only allow a list of trusted parameters through.
  def prompt_params
    params.require(:prompt).permit(:title, :description, character_ids: [], relationship_ids: [], tag_ids: [], character_trait_ids: [])
  end
end
